    ...
    inject: function (directory, core) {
        ...
        var tasks = utils.inspect(directory).map(function (file) {
            ...
            return function (cb) {
                ...
                var route = controller['$route'] || _route(relative);
                var method = controller['$method'] || 'all';

                cb(null, {
                    method: method,
                    route: route,
                    handler: _.wrap(controller, function (fn, req, res, next) {
                        var promise = fn(req, res, next);
                        if (promise && promise.then)
                            promise.then(res.send.bind(res)).catch(next);
                    })
                });
